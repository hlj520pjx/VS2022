...
当用户**对控件进行操作时**（比如：鼠标点击、键盘按下、鼠标滚动），控件就会触发一个**事件**。
而**事件参数对象（EventArgs）**，就是伴随着这个事件产生的一个“**信息包裹**”。它专门用来记录**用户到底是怎么操作控件的**。

每当用户和控件进行交互时就会产生**e事件参数对象**，例如

- **鼠标事件（MouseEventArgs）**：
  - 用户**点击了**按钮（这就是交互）。
  - 于是系统生成了 `e` 这个对象。
  - `e.X`、`e.Y` 记录了**鼠标点的哪个位置**。
  - `e.Button` 记录了**点的是左键还是右键**。
  - `e.Clicks` 记录了**是单击还是双击**。
- **键盘事件（KeyEventArgs）**：
  - 用户在输入框里**敲了键盘**（这也是交互）。
  - 系统生成了 `e` 这个对象。
  - `e.KeyCode` 记录了**按的是回车键还是ESC键**。
  - `e.Control` 记录了**有没有同时按住Ctrl键**。
