...
每个从站都有自己的id地址，在通信的时候需要选择指定的从站；每个从站都有4个数据存储区，分别代表不同的功能，这4个存储区分为：线圈状态、输入线圈 / 离散（只读）、保持型寄存器、输入寄存器（只读）。

代码使用Modbus通信，需要安装第三方库：NModbus4

![1787627586733](./assets/1787627586733.png) 
...
namespace MyModBus
{
    public partial class BeltControl : Form
    {
        public BeltControl()
        {
            InitializeComponent();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                // 创建串口、启动、创建modubus对象
                var s = new SerialPort("COM5", 9600, Parity.None, 8, StopBits.One);
                s.Open();
                master = ModbusSerialMaster.CreateRtu(s);
                MessageBox.Show("连接成功");
            }
            catch (Exception err)
            {
                MessageBox.Show($"连接失败，错误是：{err.Message}");
            }
        }
        ...
        private async void button2_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show($"启动失败，错误是：{err.Message}");
            }
        ...
        private async void button3_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show($"停止失败，错误是：{err.Message}");
            }
        }
        ...
        {
            bool IsParsed = int.TryParse(textBox1.Text, out int Speed);
            if (!IsParsed)
            {
            ...
            {
                MessageBox.Show($"设置速度失败，错误是：{err.Message}");
            }
        }
    }
}
...
![1787641679556](./assets/1787641679556.png) 