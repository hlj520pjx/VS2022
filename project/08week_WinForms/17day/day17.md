...
- 设备启动后，设定温度保存在寄存器，实际温度使用定时器每隔400ms给寄存器写数据，接近实际温度	
...
> 点击"断开连接"：关闭串口、清空主站，两个定时器停止，按钮状态还原，日志写入"设备置待机状态"。
...
3. **停止：**点击"停止"→ 停止虚拟温度定时器，按钮状态还原，日志记录"设备置待机状态"。
...
2. 以寄存器数组构造 DeviceTempRecord 实体，采集时间取当前时间 DateTime.Now；
...
> 设备启动后 也开启了两个计时器: 模拟温度变化定时器,数据记录定时器
>
...
> 模拟最高温度300度;
>
>  温度模拟逻辑设定: 到达300后温度下降,下降到30后升温, 升降温都1度1度来
...
private async void TempMoni(object sender, EventArgs e)
{
    ...
    // 设置当前温度展示
    currentTempLab.Text = CurrTemp[0] + "°C";
    // 并判断是否超过预设温度(故障,是否超温)
    // 读取寄存器中的设定温度
...
3. 点击"查询"：按时间范围查询并刷新表格（采集时间/设备状态/设定温度/实际温度/故障码，时间倒序）；
...
```C#
   ...
   // 时间范围查询数据
   private void SreachData(object sender, EventArgs e)
   {
...
> > 开始时间的最大值就是 当前的结束时间
> >
> > 结束时间的最小值当前的开始时间,结束时间的最大值 当前时间

- 当我们每次修改开始结束时间的时候, 需要将开始/结束时间的最大最小时间也做修改
  - 时间值变化事件中处理
...
private void DataInit(object? sender, EventArgs e)
{
    ...
    button1.Click += SreachData;
    button2.Click += SaveData;
    dateTimePicker1.ValueChanged += TimeChange; // 时间控件的值变化时候触发
...
```C#
 private async void SreachData(object sender, EventArgs e)
 {
...
**操作方式：**查询成功后自动计算，结果显示在窗体底部三个标签（"最大温度：xx℃"等）；无数据显示"NAN"。
...
```C#
 ...
 private void TempCala(DataTable dt)
 {
     ...
     label5.Text = $"最底温度: {MinTemp}°C";
     label6.Text = $"平均温度: {AvgTemp}°C";

     // DataTable数据.Coumpute("Max(字段)","")
...
1. 查询成功后点击"导出 JSON"；
2. 弹出保存对话框，默认文件名"yyyy-MM-dd HH-mm-ss.json"；
...
private void SaveData(object sender, EventArgs e)
{
    ...
    // dt.AsEnumerable 将数据转换为 可以LinQ操作的数据
    ...
    using (SaveFileDialog Sf = new SaveFileDialog())
    {
        ...
        Sf.FileName = DateTime.Now.ToString().Replace(":","-");
